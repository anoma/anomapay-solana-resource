...
| `7171ae21c90bef5120bb9cfe922e71bb2556df4d279cec5e0015e4aca33bb1e8` | (this commit) | 2026-09-17 | ARM v2 port: arm-risc0 `main` at `5085021f` (v2.0.0-rc.5 plus the Solana crates), anoma-pa-solana-client `28385256`, which owns the V2 SPL token forwarder's segment sizes (wrap 8 accounts, unwrap 7) and its 122-byte wrap input without the ed25519 signature; the migrate call removed (its redesign is anoma/dos-pm#42, the Solana port anoma/dos-pm#77). Reproducible Docker build with cargo-risczero 3.0.5 (builder `r0.1.88.0`), identical from two in-place builds and one from another path; CI rebuilds and compares. (Transient intra-branch values `134aaf7e…`, `6a88d368…` and `880569c1…` preceded the review cleanup, the branch-pin change and the client re-pin of the same work and never left the branch.) | None — V2 is a fresh deployment; no resource has been minted under any prior V2 key. Expected future rotations: the re-pin to the next arm-risc0 release tag, the migrate port (dos-pm#77 after dos-pm#42), anomapay-pm#58, arm-risc0#252. |

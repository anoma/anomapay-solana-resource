...
| `880569c12ea6b178d7c7c751201a8d35ca337eb34239d387ef78d4cbd7e489f8` | (this commit) | 2026-09-17 | ARM v2 port: arm-risc0 `main` at `5085021f` (v2.0.0-rc.5 plus the Solana crates), anoma-pa-solana-client `9cd3752e`; the V2 SPL token forwarder's segments (wrap 8 accounts, unwrap 7) and its 122-byte wrap input without the ed25519 signature; the migrate call removed (its redesign is anoma/dos-pm#42, the Solana port anoma/dos-pm#77). Reproducible Docker build with cargo-risczero 3.0.5 (builder `r0.1.88.0`), identical from two in-place builds and one from another path; CI rebuilds and compares. (Transient intra-branch values `134aaf7e…` and `6a88d368…` preceded the review cleanup and the branch-pin change of the same work and never left the branch.) | None — V2 is a fresh deployment; no resource has been minted under any prior V2 key. Expected future rotations: the re-pin to the next arm-risc0 release tag, the migrate port (dos-pm#77 after dos-pm#42), anomapay-pm#58, arm-risc0#252. |

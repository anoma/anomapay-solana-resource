...
| VK | Commit | Date | Reason | Migration plan |
|---|---|---|---|---|
| `93e00f30c272a747fbd48c0938ea71842e88a34db15cebc4fff3f451c320588d` | `99fc8c3` | 2026-06-24 | Initial Solana port of the v2 (migration-capable) token-transfer guest, adapted from the ERC20 v2 resource. The `Migrate` call type is added on top of the v1 wrap/unwrap logic, sourcing `OP_MIGRATE` / `encode_migrate_forwarder_input` from `anoma-pa-solana-client`. Local (non-reproducible) build. (Transient intra-branch values `c9f22b83…`, `e76734b0…`, and `63eb0650…` appeared while iterating on versioning and the encoder source; they never left this branch and are omitted.) | None — not deployed. v2 has never shipped to mainnet, so no resources are minted under this VK and nothing is orphaned. The migration plan must be revisited (including a reproducible docker build of the guest) before this VK ships to a live network. |
| `725520e56ee1d2e6ec444788a87fd97aae15a9332386fdc360260345136a010b` | (this commit) | 2026-09-17 | ARM v2 port: arm-risc0 `main` at `5085021f` (v2.0.0-rc.5 plus the Solana crates), anoma-pa-solana-client `5b2df947`, which owns the V2 SPL token forwarder's segment sizes (wrap 8 accounts, unwrap 7) and its success return byte and its 122-byte wrap input without the ed25519 signature; the migrate call removed (its redesign is anoma/dos-pm#42, the Solana port anoma/dos-pm#77). Reproducible Docker build with cargo-risczero 3.0.5 (builder `r0.1.88.0`), identical from two in-place builds and one from another path; CI rebuilds and compares. (Transient intra-branch values `134aaf7e…`, `6a88d368…`, `880569c1…` and `7171ae21…` preceded the review cleanups, the branch-pin change and the client re-pins of the same work and never left the branch.) | None — V2 is a fresh deployment; no resource has been minted under any prior V2 key. Expected future rotations: the re-pin to the next arm-risc0 release tag, the migrate port (dos-pm#77 after dos-pm#42), anomapay-pm#58, arm-risc0#252. |

## Process for adding a new VK

1. Rebuild the guest as [`BUILDING_THE_VK.md`](BUILDING_THE_VK.md) describes, write the new image ID to `src/lib.rs::TOKEN_TRANSFER_ID`.

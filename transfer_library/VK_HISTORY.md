...
| `6a88d368d3155fa785301807417e2624095732a2622d2bc8eb0f03bf4f6e877c` | (this commit) | 2026-09-17 | ARM v2 port: arm-risc0 `main` at `5085021f` (v2.0.0-rc.5 plus the Solana crates), anoma-pa-solana-client `9cd3752e`; the V2 SPL token forwarder's segments (wrap 8 accounts, unwrap 7) and its 122-byte wrap input without the ed25519 signature; the migrate call removed (its redesign is anoma/dos-pm#42, the Solana port anoma/dos-pm#77). Reproducible Docker build with cargo-risczero 3.0.5 (builder `r0.1.88.0`), identical from two in-place builds and one from another path; CI rebuilds and compares. (A transient intra-branch value `134aaf7e…` preceded the review cleanup of the same change and never left the branch.) | None — V2 is a fresh deployment; no resource has been minted under any prior V2 key. Expected future rotations: the re-pin to the next arm-risc0 release tag, the migrate port (dos-pm#77 after dos-pm#42), anomapay-pm#58, arm-risc0#252. |
